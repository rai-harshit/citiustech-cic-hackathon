...
def detect_img(yolo):
	...
	with open("./static/prediction.txt","w") as f:
		for items in predicted_pose_sequence:
			f.write("%s\n"%items)
	x = open("./static/description.txt","w")
	x.write(description)
	x.close()
	yolo.close_session()
...
def build_description(predicted_pose_sequence,timestamps):
	description = ""
	ideal_pose_sequence = "012346543210" #represents complete suryanamaskar
	...
	sn_start = -100
	sn_end = -100
	for i,j in zip(timestamps,current_sequence):
		if j==0: 
			if sn_start == -100:
				sn_start = i
			else:
				sn_end = i
	description+="The Suryanamaskar starts at {} secs. and ends at {} secs.".format(sn_start,sn_end)
	no_human = 0
	for i in predicted_pose_sequence:
		if i==-2:
			no_human += 1
	description+="Out of all {} key frames, human was not present in {} frames.".format(len(predicted_pose_sequence),no_human)
